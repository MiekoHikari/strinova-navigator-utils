	...
	public async run(interaction: ModalSubmitInteraction, parsed: { step: 'identity' | 'details' }) {
		...
		if (parsed.step === 'identity') {
			...
			const continueBtn = new ButtonBuilder()
				.setCustomId('report:continue-details')
			...
			return interaction.reply({
				content: priorCount > 0 ? `Target has ${priorCount} prior report(s). Click Continue to add details.` : 'Identity captured. Click Continue to add report details.',
				components: [new ActionRowBuilder<ButtonBuilder>().addComponents(continueBtn, cancelBtn)],
				ephemeral: true
			});
		}

		if (parsed.step === 'details') {
			const catsRaw = interaction.fields.getTextInputValue('categories')?.trim();
			...
			if (!catsRaw) return interaction.reply({ content: 'Categories required.', ephemeral: true });
			if (!summary) return interaction.reply({ content: 'Summary required.', ephemeral: true });
			const categories = catsRaw.split(',').map(c => c.trim().toUpperCase()).filter(Boolean);
			// Validate categories against allowed set
			const invalid = categories.filter(c => !PLAYER_REPORT_CATEGORIES.includes(c as any));
			if (invalid.length) {
				return interaction.reply({ content: `Invalid categories: ${invalid.join(', ')}. Allowed: ${PLAYER_REPORT_CATEGORIES.join(', ')}`, ephemeral: true });
			}
			...
			await PlayerReportModel.updateOne({ _id: session._id }, { $set: { categories, matchId: matchId || undefined, summary }, $unset: { expiresAt: 1 } });
			...
			const catsShort = categories.slice(0, 3).join(', ');
			...
			try {
				...
				const thread = await (parentChannel as any).threads.create({
					...
					message: {
						content: `Reporter: <@${interaction.user.id}>
Reported: ${targetLabel}
Categories: ${categories.join(', ')}${matchId ? `\nMatch ID: ${matchId}` : ''}
Summary:\n${summary}\n\nPlease upload any evidence (screenshots, videos, logs) in this thread. Staff will review as soon as possible.`
					}
				});
