...
export class AttemptFetchID extends ScheduledTask {
	...
	public async run(payload: fetchIDPayload) {
        ...
        if (!entry.performedByUsername) return this.container.logger.warn(`[Stardust][AttemptFetchID] Mod Action entry ${entry.id} has no performedByUsername to fetch ID from.`);

        // Skip if username has # discriminator (cannot search by that)
        if (entry.performedByUsername.includes('#')) {
            return this.container.logger.warn(`[Stardust][AttemptFetchID] Mod Action entry ${entry.id} has a discriminator in performedByUsername (${entry.performedByUsername}), skipping fetch.`);
        }
        ...
        if (existingEntry) {
            ...
            await prisma.modAction.update({
                ...
                data: {
                    moderator: {
                        connect: {
                            userId: existingEntry.moderatorId!
                        }
                    },
                }
            });
        ...
        if (!member.size) {
            this.container.logger.warn(`[Stardust][AttemptFetchID] Could not find member with username ${entry.performedByUsername} for Mod Action entry ${entry.id}.`);
            return;
        }
        ...
        this.container.logger.info(`[Stardust][AttemptFetchID] Successfully updated Mod Action entry ${entry.id} with moderator ID ${userId}.`);
    }
}
